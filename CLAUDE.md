...
- Elements carry **no `edges`** — connections are implied by grid geometry. Derive them once: bucket `node.position.x/y` against the chapter's `meta.timelineData.columns[].width` / `rows[].height` to get (column, row), then apply the standard layout rules (READMODEL ← EVENT same column; EVENT ← COMMAND same column; COMMAND ← READMODEL previous column).
