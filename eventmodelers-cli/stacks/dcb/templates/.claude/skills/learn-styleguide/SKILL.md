...
description: This project's visual style guide / design system reference material — read before generating or redesigning any UI in web/ (screens, components). Populated and kept up to date by the init-style-guide skill.
---
...
The design system itself is `web/src/styles/design-system.css`: its tokens (`bg-primary`,
`text-muted-foreground`, `border-input`, …) and its `mock-*` classes are what the app and the
board's wireframes share. Style with those tokens and the shadcn/ui components in
`web/src/components/ui/`, never with hard-coded colors.

Before turning a screen's mockup into components, or designing any new screen, read whichever file(s) under `references/` are relevant to
