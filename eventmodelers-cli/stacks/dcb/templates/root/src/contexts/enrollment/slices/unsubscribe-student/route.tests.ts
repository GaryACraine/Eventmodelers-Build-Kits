...
describe("POST /unsubscribe-student-from-course — unsubscribe student", () => {
    test("unsubscribes student and returns 204", async () => {
        await spec
            ...
            .when(agent => agent.post("/unsubscribe-student-from-course").send({ courseId: "c1", studentId: "s1" }))
            .then(
                expectResponse(204),
    ...
    test("returns 404 when course does not exist", async () => {
        await spec
            .when(agent => agent.post("/unsubscribe-student-from-course").send({ courseId: "nonexistent", studentId: "s1" }))
