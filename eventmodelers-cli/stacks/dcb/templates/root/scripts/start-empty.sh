...
# The example's frontend goes too: web/'s pages (all but Home), its slices, and the generated API types, and so
# does its API contract (api/openapi.json): your model's export writes yours.
set -euo pipefail
cd "$(dirname "$0")/.."
